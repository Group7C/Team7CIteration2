...
class _AddTaskModalState extends State<AddTaskModal> {
  ...
  void submitTask() async {
    if (formKey.currentState!.validate()) {
      // Get assignee user ID from the username
      String assigneeId = '';
      String assigneeName = taskMembers.keys.isNotEmpty ? taskMembers.keys.first : 'Unassigned';
      
      try {
        // Get the user ID for the assignee by username
        if (taskMembers.isNotEmpty) {
          final response = await http.get(
            Uri.parse('http://127.0.0.1:5000/get/user/id?email=$assigneeName'),
          );
          
          if (response.statusCode == 200) {
            assigneeId = response.body.toString();
          }
        }
        
        // If no assignee found, use current user ID
        if (assigneeId.isEmpty) {
          final usser = context.read<Usser>();
          if (usser.usserID.isEmpty) {
            await usser.getID();
          }
          assigneeId = usser.usserID;
        }
        ...
        final requestData = {
          ...
          'assignee_id': int.tryParse(assigneeId) ?? 0,
          ...
          'notification_frequency': notificationFrequencyNotifier.value.name,
        };
  ...
  // Formats enum values to friendly display text
