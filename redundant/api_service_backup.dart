...
class ApiService {
    ...
    try {
      ...
      for (var project in projects) {
        ...
        if (response.statusCode == 200) {
          ...
          // Add tasks to the list with project info
          for (var task in projectTasks) {
            final formattedTask = {
              ...
              'status': task['status'] ?? 'todo',  // Use the status from the backend
  ...
  // Fetch user's groups (placeholder - you'll need to implement the API endpoint)
  static Future<List<Map<String, dynamic>>> fetchUserGroups(BuildContext context) async {
    print('\n=== FETCHING USER GROUPS ===');
    print('Note: Group endpoint not implemented yet');
    print('=== GROUPS FETCH SKIPPED ===\n');
    return [];
  }
